...
- **Transient-Cache** für die Default-Dauer von 12 h, konfigurierbar 1–168 h.
...
**4.5.5 CSS-Strategie.** Inline-First: Alle Styles werden als `<style id="ttt-inline-critical">`-Block im Shortcode-Output geschrieben, damit Page-Builder/Cache-Plugins das Layout nicht abräumen können. `assets/style.css` enthält denselben Regelsatz als Backup. Beide Quellen nutzen das gleiche `--ttt-*`-Token-Set, mit theme.json-Fallbacks für Brand-Farben. Status-Farben sind Plugin-fix.
...
**4.7.1 Sprache der Frontend-Strings.** Aktuell deutsch hardcoded im Code, aber `__()`/`esc_html__()`-konform mit Text-Domain `training-translation-tracker`. Eine `.pot`-Datei steht noch aus (siehe Arbeitsplan beim Maintainer).
...
│   ├── assets/
│   │   ├── style.css            # External-CSS (Backup zu Inline)
...
├── sync-schemas.py              # Schema-Sync zwischen ../Konzept/ und action/
├── CONTRIBUTING.md
├── README.md                    # Mono-Repo-Übersicht
└── LICENSE
```

### Was beim Maintainer lokal daneben liegt

```
Maintainer-Arbeitsordner/
├── GitHub/                      # ← der oben gezeigte Repo-Root
└── Konzept/                     # NICHT im Repo
...
Konzept lebt **außerhalb** des Repos (Sibling von `GitHub/`). Die JSON-Schemata in `../Konzept/schemas/` sind die Source-of-Truth und werden via `sync-schemas.py` mit `action/schemas/` gespiegelt. Contributors, die das Repo standalone clonen, sehen Konzept gar nicht und brauchen es auch nicht — sie editieren `action/schemas/` direkt.
