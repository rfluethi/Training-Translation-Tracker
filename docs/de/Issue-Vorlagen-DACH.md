...
Ein sechster Wert `unset` existiert im Datenmodell, wird aber von der Action vergeben, nicht von Hand. Die Action setzt ihn für Komponenten aus Issues, die noch keine geparste Status-Tabelle haben (z. B. weil die Vorlage noch nicht gefüllt wurde). Das Frontend zeigt solche Komponenten neutral grau an, damit sie sich von expliziten "open"-Komponenten unterscheiden.

### Creator / Reviewer

- GitHub-Benutzername, mit oder ohne `@`-Präfix. Beides funktioniert: `rfluethi` und `@rfluethi`, der Parser entfernt das führende `@` automatisch.
- Mehrere Personen durch Komma trennen: `rfluethi, Ursha-wp` oder `@rfluethi, @Ursha-wp`.
