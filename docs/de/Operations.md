...
|---|---|---|
| Action-Code | Repo `Training-Translation-Tracker`, Branch `main` | Maintainer + Contributors |
...
Verlauf beobachten unter `Actions → release-plugin.yml`. Bei grünem Workflow: Release ist verfügbar unter `https://github.com/<owner>/Training-Translation-Tracker/releases/tag/v0.2.5`.
...
1. **Plugins → Plugin hochladen**, ZIP aus dem aktuellen Release-Tab wählen.
2. Im Dialog, den WordPress bei einem bereits installierten Plugin zeigt, **«Aktuelle durch hochgeladene ersetzen»** bestätigen. Einstellungen und Cache bleiben erhalten.
3. **Einstellungen → Translation Tracker**, URL und Cache-Dauer prüfen.
4. **Cache jetzt leeren** drücken.
5. Test-Seite öffnen, prüfen ob Dashboard erscheint.

**Nie über vorheriges Löschen aktualisieren.** Löschen führt `uninstall.php` aus, das absichtlich alle Einstellungen und Cache-Daten entfernt. Deaktivieren allein ist harmlos, aber unnötig; der Ersetzen-Dialog erledigt alles.
...
`--force` ersetzt die installierte Version an Ort und Stelle, wie der Ersetzen-Dialog; die Einstellungen bleiben erhalten.

### Kein automatischer Updater (Beschluss)

Es gibt bewusst keinen Update-Mechanismus im Plugin. Die Bibliothek `plugin-update-checker` wurde in 0.4.2 entfernt (sie ist selbst ein Updater, den Plugin Check verbietet, und erzeugte Dutzende Störbefunde); am 2026-08-26 wurden die verfügbaren GitHub-Updater-Varianten geprüft und verworfen. Updates laufen manuell über das Release-ZIP und den Ersetzen-Dialog wie oben beschrieben. Der Plugin-Header `Update URI:` bleibt gesetzt, damit wordpress.org nie ein fremdes Plugin mit gleichem Slug als «Update» anbieten kann (siehe [Architektur.md](Architektur.md), verbindliche Entscheidungen).
...
1. Repo öffentlich anlegen unter `<owner>/Training-Translation-Tracker`.
