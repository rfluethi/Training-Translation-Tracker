...
Die Action im Repo `Training-Translation-Tracker` läuft auf drei Auslösern:

- **Schedule**, `cron: "0 */12 * * *"` (alle 12 Stunden).
- **`workflow_dispatch`**, manueller Knopf in der GitHub-UI.
- **`push`** auf `main` mit Filter auf `scope.yml`, `component-templates.yml` und `status-map.yml`, sofortiges Re-Build nach Konfigurations-Änderungen.
...
4. **Matching**, Inventory-Items werden per kanonischer URL mit Issues gemacht. Items ohne Issue bleiben mit Status `open`.
...
- `overall_status`: seit 0.5.0 aus dem Projects-V2-Board-Status abgeleitet (siehe `status-map.yml`), der Komponenten-Rollup bleibt der Fallback.
...
Implementiert in der Action. Seit 0.5.0 zweistufig:

```
1. Board-Status führt: trägt das Issue des Items einen Projects-V2-
   Board-Status, der in action/status-map.yml zugeordnet ist, gewinnt
   dieser Wert. Standard-Zuordnung:
...
2. Fallback (kein Issue, kein Board-Status oder unbekannter Wert):
   Komponenten-Rollup —
   a. Alle Komponenten = "na"   → "na"
   b. Alle Nicht-na    = "done" → "done"
   c. Mind. ein "review"        → "review"
   d. Mind. ein "wip"           → "wip"
   e. Sonst                     → "open"
```

Der Board-Status ist der Status, den das Team tatsächlich pflegt, darum führt er das Dashboard; ein veröffentlichtes Item zählt auch dann als "published", wenn optionale Komponenten bewusst nicht übersetzt wurden. Die Zuordnung ist Konfiguration (`status-map.yml`, validiert gegen `schemas/status-map.schema.json`); eine umbenannte Board-Spalte braucht eine Config-Änderung, keinen Code-Eingriff. Das Plugin zeigt den Legacy-Rollup-Wert `done` als "published" an.
...
**4.2.1 `overall_status`: der Board-Status führt (0.5.0).** Der Projects-V2-Board-Status, zugeordnet über `action/status-map.yml`, bestimmt `overall_status`; der Komponenten-Rollup (siehe Sektion 3) ist der Fallback für Items ohne Issue, ohne Board-Status oder mit unbekanntem Wert. Begründung: Das Board ist der Status, den das Team pflegt, und ein veröffentlichtes Item muss als veröffentlicht zählen, auch wenn optionale Komponenten unübersetzt bleiben. `overall_status` wird nur für Stats-Aggregation und Filterung verwendet.
...
- `workflow_dispatch`, manueller Knopf in der GitHub-UI.
- `push` auf `main` mit Filter `paths: ['scope.yml', 'component-templates.yml', 'status-map.yml']`, sofortiges Re-Build nach Konfigurations-Änderungen.
...
**4.5.6 `Update URI:`-Header bleibt, obwohl Plugin Check ihn meldet (0.4.11).** WordPress identifiziert Plugins beim Update-Check nur über den Ordnernamen auf wordpress.org. Würde dort jemand ein fremdes Plugin unter dem Slug `training-translation-tracker` einstellen, böte WordPress es unseren Installationen als «Update» an. Genau das unterbindet der Header. Plugin Check prüft die Regeln für wordpress.org-gehostete Plugins, dort wäre der Header verboten; für ein GitHub-verteiltes Plugin ist er die empfohlene Absicherung. Der Befund ist ein erwarteter Dauerbefund, kein Mangel. Erst eine wordpress.org-Submission würde den Header entfernen.

**4.5.7 Plugin-Check-Befunde «Offloading» sind erwartete Dauerbefunde.** Der Sniff meldet die `raw.githubusercontent.com`-URLs (Default-URL, Allow-Liste, Platzhalter). Gemeint sind ausgelagerte Frontend-Assets; hier sind die URLs die serverseitige Datenquelle mit Cache, das Kernkonzept des Plugins. Kein Handlungsbedarf.

**4.5.8 Kein automatischer Updater (0.4.2, bestätigt 2026-08-26).** `plugin-update-checker` wurde entfernt: Die Bibliothek ist selbst ein Update-Mechanismus, den Plugin Check verbietet, und erzeugte Dutzende Störbefunde. Am 2026-08-26 wurden die verfügbaren GitHub-Updater-Varianten geprüft und verworfen. Updates laufen manuell per Release-ZIP über den Ersetzen-Dialog, siehe [Operations.md](Operations.md). Preis: Fixes erreichen eine Site nur, wenn jemand handelt.

**4.5.9 Avatare abschaltbar (0.5.1).** Einstellung «GitHub-Avatare» (Standard an). Abgeschaltet rendert das Frontend Initialen, und der Besucher-Browser stellt keine Anfrage an github.com. Grund: Der einzige Drittanbieter-Abruf im Besucher-Browser (Besucher-IP an GitHub beim Öffnen eines Popovers) muss ohne Code-Eingriff vermeidbar sein. Preis: Initialen statt Gesichter, eine Einstellung mehr.
...
| Plugin-Checker neuer Versionen bringt zusätzliche Regeln | mittel | gering | Bei jedem Release einmal durchlaufen lassen; erwartete Dauerbefunde siehe Abschnitt 4.5.6/4.5.7 |
| Board-Disziplin lässt nach (Status nicht gepflegt) | mittel | mittel | Seit 0.5.0 führt der Board-Status das Dashboard; falsche Zahlen fallen dem Team direkt auf |
| Drift zwischen EN- und DE-Doku | mittel | gering | Beide Sprachen liegen nebeneinander unter `docs/en/` und `docs/de/`; jede Doku-Änderung zieht beide nach, PR-Review erzwingt es |
