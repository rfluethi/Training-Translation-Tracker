...
#### `ttt_tracker_url_allowed_hosts` (seit 0.4.8)

Allow-Liste der Hosts, auf die das Tracker-URL-Setting zeigen darf. Default ist nur `raw.githubusercontent.com`. Erweitern, wenn die `tracker.json` selbst gehostet wird auf einem Mirror oder CDN.
...
URLs, deren Host nicht auf der Liste steht oder die kein HTTPS sind, werden beim Speichern verworfen und der vorher gespeicherte Wert bleibt. Ein Admin-Hinweis erklärt die Ablehnung.
